...
class MCEWrapper:
    ...
    def search_with_ponder(self, board, wtime, btime, winc, binc, ponder, draw_offered):
        ...
        if movetime is not None:
            movetime = float(movetime)/1000
        time_limit = chess.engine.Limit(white_clock=wtime/1000, black_clock=btime/1000, white_inc=winc/1000, black_inc=binc/1000, depth=cmds.get("depth"), nodes=cmds.get("nodes"), time=movetime)
        return self.search(board, time_limit, ponder, draw_offered)
